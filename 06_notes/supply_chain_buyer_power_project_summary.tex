...
\begin{quote}
• 中文：国内生产网络中的投入需求楔子、上游暴露与信贷配置。

• 英文：Input-Demand Wedges, Upstream Exposure, and Credit Allocation in Domestic Production Networks。

• 更政策化版本：Buyer-Side Distortions and Credit Allocation in Production Networks。
\end{quote}
...
这个题目比``优惠信贷 + 高 markup + markdown + network + welfare decomposition''的菜单式扩展更干净。它把研究对象从``高利润行业为什么得到补贴''转向``下游 buyer-side distortion 是否通过生产网络造成上游暴露，以及信贷配置是否回应这种暴露''。
...
Liu 的理论主线不是狭义的金融摩擦，而是一般性市场不完善或部门扭曲如何通过投入产出联系在生产网络中累积，并影响最优部门性产业政策。金融摩擦可以是该框架的一个解释或应用场景，但不能把 Liu 缩窄为金融摩擦文章。

本项目的对应结构是：下游 buyer-side distortion 或 domestic input-demand wedge 通过 IO 网络映射为上游暴露，造成上游供应商现金流、账期、融资和投资压力；如果现实中的信贷配置回应这种暴露，则可以讨论政策是否具有生产网络意义上的二阶最优含义。
...
\bottomrule\noalign{}
\endlastfoot
政策与 upstream exposure 显著相关 & Buyer-Side Distortions + Credit Allocation 主线 & 最理想，说明政策可能针对生产网络中的买方势力暴露 \\
政策与 network centrality 相关，但与 wedge 无关 & Weak Links in Production Networks & 重点转为网络关键环节与补链强链 \\
政策与融资约束相关，但与 wedge 无关 & Networked Financial Frictions & 主机制是上游融资摩擦，不是 markdown \\
