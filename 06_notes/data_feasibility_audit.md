...
定位边界：本节只说明中征平台数据能否作为 `policy_support_i_u_t` 的候选 proxy。它不改变项目主线。当前项目的主线仍是 downstream input-demand wedge、upstream exposure 和 credit allocation；中征平台数据不能把文章改写成供应链金融文章。更准确地说，中征平台不是主 treatment，而是观察供应链金融是否成为一种政策工具的辅助数据源。
...
| 表 | 可用方式 | 谨慎边界 |
|---|---|---|
| Table 4 | 不建议作为主变量。Table 4 应先用 NTSD 财务变量检验 high-exposure upstream stress | 不能让 SCF access 替代 upstream stress 本身 |
| Table 5 | 若能按企业或行业聚合 `SCF_access_i_t`，可作为 `policy_support_i_u_t` 的候选 proxy，检验 platform access 是否更多出现在 high-exposure upstream firms/sectors | 这只是 targeting of access，不是 actual SCF flow |
| Table 6A | 若有加入时间，可作为补充政策变量检验 access 后现金流、投资、R&D、专利是否改善 | 有效性解释弱于 credit subsidy 或 actual financing amount |
| Table 6B | 若有 AR days、应收账款、buyer dependence，可作为补充检验 trade-off / payment persistence | 若无债权人-债务人关系，不能声称 buyer-supplier financing network |

当前判断：中征平台数据是 supplementary candidate SCF proxy。当前样本只能支持平台参与主体识别，不能支持 actual financing flow。完整可用性取决于是否能拿到加入时间和逐笔交易字段。无论完整数据是否可用，它都不应替代本文的核心机制：input-demand wedge 经生产网络传导为 upstream exposure，并影响 credit allocation 与 policy effectiveness。
